...
output:
  ...
  pdf_document:
    toc: yes
---
1. Tidyverse, dplyr, select and filter
Tidyverse is a libary that has lots of packages, within dplyr: filter and select are within it
2. select(): you can select variables/ columns of interest to make a dataframe 
3.filter(): filter can be use to filter out specific values (filter works in rows, it is search across dataframe)
...
```{r}
fish$lakeid <- as.factor(fish$lakeid) #a factor can be something like carivore or herbivore cz there's only two variables within "type of animal"
levels(fish$lakeid)
```
...
```{r}
filter(fish, lakeid == "AL" | length > 350) #vertical line stands for "or", so interested in the information from lakeid only from "AL" and only from fishes that are bigger than 350, so it'll have multiple lakeid other than AL
```
...
filter(mammals2, order=="Primates")
