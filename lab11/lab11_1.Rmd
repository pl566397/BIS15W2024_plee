...
life_history %>% 
  group_by(order) %>% #get your x
  summarize(mean_mass=mean(mass, na.rm=T)) %>% #gets your y
  ggplot(aes(x=reorder(order, mean_mass), y=mean_mass))+ #reorder makes it so the x-axi is arranged by mean_mass in greatest to least 
  geom_col()+
  coord_flip()+
  scale_y_log10()
```
...
life_history %>%
  ...
  scale_y_log10() #overlooks outliers
```
...
elephants %>% 
  ...
  geom_smooth(method=lm, se=F)#gives you a linear regression line
```
...
elephants %>% 
  ggplot(aes(x=age, y=height)) + 
  geom_point() + 
  geom_smooth(method=lm, se=F)+ #se is standard error, so it's going to add the SE along with the line =T
  labs(title="Elephant Age vs. Height", #this adds a title to the plot
       x="Age(years)", #what we want x axis to be called
       y="Height(cm)")+
...
```{r}
elephants %>% 
  ggplot(aes(x=sex, fill=sex))+ #fill is a grouping option, itis filling the bar by sex
  geom_bar()
```
...
```{r}
life_history %>% 
  ggplot(aes(x=gestation, y=log10(mass), size=mass))+ #plotted gestation vs. mass, made size of plot relative to the mass of the point plotted
  geom_point(na.rm=T)
```
