...
my_vector_sequence #R thinks of position, not numbers
```
...
```{r}
my_vector_sequence<=10 #<= means less than or equal to 10
```
...
```{r}
my_vector_sequence[my_vector_sequence<=10] #you can use the brackets but you will have to tell R to look into my vector sequence and look for numbers less than or equal to 10
```
...
harry_potter_matrix #matrix is a bunch of stacked vectors, to access 317.5 you would put [1,1]
```
...
```{r}
total_earnings <- colSums(all_harry_potter_matrix) #doing a summation on the columns
total_earnings
```
...
```{r}
rbind(all_harry_potter_matrix, total_earnings) #instead of cbind, use rbind to add a new row
```
...
```{r}
harry_potter_matrix[2,1] #checking which number is in this position
```
