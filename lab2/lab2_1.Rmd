...
```
# = and arrow means different things so make sure I use the gets symbol and not the equal. 
...
```{r}
(venom_GT+chiron+veyron)/3 #arithmethic means use numbers
```
...
There is nothing to intrepret because there isn't a result. NAs means there is missing data. Even if you have a million data but there is one NA, it will not calculate the data.
4. Recalculate the mean using the following code chunk. Why is the useful?  
```{r}
mean(new_vector, na.rm=T) #na.rm removes the NA values in the vector equals True. F=False will give you NA again.
```
