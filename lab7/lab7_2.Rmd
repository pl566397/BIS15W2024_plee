...
```{r}
penguins %>% 
  group_by(species, island) %>% #only words with categorical variables
...
```{r}
penguins %>% 
  count(sex, island) #NA means there was a problem recording the data
```
...
```{r}
penguins %>%
  summarize(across(c(species, island, sex), n_distinct))#count the # of distinct observations
```
...
```{r}
penguins %>%
  summarize(across(contains("mm"), \(x) mean(x, na.rm = TRUE))) #\(x), use it to correct the error that pops up above
```
