...
library("tidyverse") #authors put their data up onto github, so other scientists can go to look at the results and repeat their experiments to see if they can make their own data
```
...
```{r}
str(hot_springs) # tells us scientists as a character and etc
```
...
hot_springs$spring <- as.factor(hot_springs$spring) #factor has discrete levels 
```
...
```{r}
summary(fish) #tells us how big data is, gives us min, medium, max, can't do that to characters
```
...
```{r}
head(fish) #gives you first few observations of dataframe
```
