...
1.  The `ui` function controls the user inputs and the way the app will be displayed; i.e. this controls how your app looks.(##controls how your app looks)
2.  The `server` function is the part of the app which takes the values of the user inputs, performs calculations and/or makes plots, and prepares the outputs for display; i.e. this controls how your app works. (##controls what the app does)
3.  The `run` function combines the ui and server function to run the app.  (##combines both together)
...
```{r, eval=F, echo=T}
#ui=drop down menu, button, slider
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm")#refers to drop down menu "select input", select what variable you want on the x axis, made dropdown menu of these options
)
...
```{r, eval=F, echo=T}
#space out the code so you don't forget anything like comma
ui <- fluidPage(
...
selectInput("y", "Select Y Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm") #not changing my choices because I am plotting the same thing, have the user inface set up
...
penguins %>% 
  ...
  geom_point() #plot we want to make
```
...
ui <- fluidPage(
    ...
    plotOutput("plot", width="500px", height="400px") #providing the size/aspect of the visual that we are producing
)
