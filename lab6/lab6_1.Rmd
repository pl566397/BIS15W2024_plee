...
```{r}
fish %>% 
  mutate(length_mm = length*10) %>% #making a new variable, taking length and multiplying it by 10
...
```{r}
mammals %>%
  mutate_all(tolower)#making eveything lowercase
```
...
mammals %>% 
  select(genus, species, newborn) %>%
  mutate(newborn_new = ifelse(newborn == -999.00, NA, newborn))%>% #if else allows you to replace -999 with NAs
  arrange(newborn)
```
